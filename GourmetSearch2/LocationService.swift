...
//  GourmetSearch2
//
//  Created by HideakiTouhara on 2017/09/08.
...
public extension Notification.Name {
    ...
    public static let authorized = Notification.Name("Authrized")
...
public class LocationService: NSObject, CLLocationManagerDelegate {
    ...
    // 位置情報がONになっていないダイアログ
    public var locaionServiceDisabledAlert: UIAlertController {
        get {
            let alert = UIAlertController(title: "位置情報が取得できません", message: "設定からプライバシー → 位置情報画面を開いてGourmetSearchの位置情報の許可を「このAppの使用中のみ許可」と設定してください。", preferredStyle: .alert)
            alert.addAction(
