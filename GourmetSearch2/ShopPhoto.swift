...
public class ShopPhoto {
    ...
    private func load() {
        ...
        ud.register(defaults: ["photos": [String: [String]](), "names": [String: String](), "gids": [String]()])
        ud.synchronize()
    ...
    public func append(shop: Shop, image: UIImage) {
        ...
        // UIImageからJPEGデータ作成
        guard let date = UIImageJPEGRepresentation(image, 0.8) else {
            return
        }
        // ファイル名作成
        ...
        // 書き込み
        do {
            try date.write(to: fileURL, options: .atomic)
        } catch {
            print(error)
            return
        }
